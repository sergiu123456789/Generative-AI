...
Fine-tuned Meta’s LLaMA language model using **LoRA (Low-Rank Adaptation)** for parameter-efficient training, leveraging the Hugging Face ecosystem.
...
1. **Dataset Preparation** – Cleaned and formatted training data for fine-tuning  
2. **LoRA Integration** – Applied LoRA adapters to reduce trainable parameters  
3. **Training** – Used Hugging Face Trainer and TRL libraries for training loops  
4. **Evaluation** – Tested on downstream tasks for instruction adherence and accuracy

## ✅ Results
- Fine-tuning LLaMA with reduced computational overhead
